...
orig_weights_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_weights_dogma_t_1_dst.hdf5')  # where weights will be saved
orig_json_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_model_dogma_t_1_dst.json')

save_model = True  # if weights will be saved
weights_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_weights_dogma_t_5_dst.hdf5')  # where weights will be saved
json_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_model_dogma_t_5_dst.json')
...
# tensorboard
tensorboard = TensorBoard(log_dir="logs_dst_pred_5/{}".format(time()), histogram_freq=1, write_graph=False, write_grads=False, write_images=False, batch_size=1)
...
plt.show()
plt.savefig('loss_full_kitti_dogma_t_5_dst.png')

# save history in a hickle file
hkl.dump(history.history, 'history_full_kitti_dogma_t_5_dst.hkl', mode='w')
