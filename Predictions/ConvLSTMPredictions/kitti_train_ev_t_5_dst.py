...
orig_weights_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_weights_ev_t_1_dst.hdf5')  # original t+1 weights
orig_json_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_model_ev_t_1_dst.json')

save_model = True  # if weights will be saved
weights_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_weights_ev_t_5_dst.hdf5')  # where weights will be saved
json_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_model_ev_t_5_dst.json')

# Data files
DATA_DIR_OCC = "../../Data/PredNetData/EvidentialGridsDST/"
...
# tensorboard
tensorboard = TensorBoard(log_dir="logs_dst/{}".format(time()), histogram_freq=1, write_graph=False, write_grads=True, write_images=True)
...
plt.show()
plt.savefig('loss_full_kitti_ev_t_5_dst.png')

# save history in a hickle file
hkl.dump(history.history, 'history_full_kitti_ev_t_5_dst.hkl', mode='w')
