...
weights_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_weights_ev_t_1_dst.hdf5')  # where weights will be saved
json_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_model_ev_t_1_dst.json')
...
# tensorboard
tensorboard = TensorBoard(log_dir="logs_check/{}".format(time()), histogram_freq=1, write_graph=True, write_grads=True, write_images=True, batch_size=1)
...
plt.show()
plt.savefig('loss_full_kitti_ev_t_1_dst.png')

# save history in a hickle file
hkl.dump(history.history, 'history_full_kitti_ev_t_1_dst.hkl', mode='w')
