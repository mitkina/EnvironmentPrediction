...
OUTPUT_DIR_DOGMA = "../../Data/PredNetData/DOGMaDST/"
OUTPUT_DIR_EVIDENTIAL = "../../Data/PredNetData/EvidentialGridsDST/"
...
def process_data():
	...
	for split in splits:
		...
		for name in splits[split]:
			...
			# combine masses with velocities
			grid = np.concatenate((EVIDENTIAL[:,:,:,0:2,0], DOGMA), axis=3)

			if not found:
				X = grid
				X_ev = EVIDENTIAL[:,:,:,0:2,0]
				found = True

			else:
				X = np.concatenate((X, grid), axis=0)
				X_ev = np.concatenate((X_ev, EVIDENTIAL[:,:,:,0:2,0]), axis=0)
		...
		X[:,:,:,3] = (X[:,:,:,3] - min_vel)/(max_vel-min_vel)
