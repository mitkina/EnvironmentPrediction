...
orig_weights_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_weights_dogma_t_1.hdf5')  # where weights will be saved
orig_json_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_model_dogma_t_1.json')

save_model = True  # if weights will be saved
weights_file = os.path.join(WEIGHTS_DIR, 'prednet_full_kitti_weights_dogma_t_5.hdf5')  # where weights will be saved
...
# tensorboard
tensorboard = TensorBoard(log_dir="logs_dogma/{}".format(time()), histogram_freq=1, write_graph=False, write_grads=False, write_images=True)
...
plt.show()
plt.savefig('loss_full_kitti_dogma_t_5.png')

# save history in a hickle file
hkl.dump(history.history, 'history_full_kitti_dogma_t_5.hkl', mode='w')
